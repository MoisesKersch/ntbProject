...
import br.com.adaptaconsultoria.amr.proxy.CartaoCieloConfirmacaoProxy;
 ...
 */
@Service
public class CartaoCieloService {
  ...
  public PreCompra confirmaTransacao(PreCompra preCompra) throws Exception {
    ...
    if (config == null)
      throw new Exception(
          "Nenhuma configuração para autorização de cartões com a cielo encontrada para a organização da compra prévia número: "
              + preCompra.getNumero());

    CartaoCieloConfirmacaoProxy proxy = new CartaoCieloConfirmacaoProxy();
    RetCieloConfirma ret = proxy.doGet(config, preCompra);
  ...
  public static void main(String[] args) throws Exception {
    CartaoCieloService service = new CartaoCieloService();
    PreCompra preCompra = DaoFactory.getInstance().getPreCompraDao()
        .carregaPorId("B7B5A639BA464E74850A3D2A9AF33F93");
